...
def _preparar_tabla_metricas_detalle(
    ...
    altura_scroll = _altura_scroll_tabla_metricas(n_filas, clase_css, tabla_font_size, viewport_h)
...
def fig_ranking_barras(
    ...
    layout_kw: dict[str, Any] = dict(
        ...
        bargap=0.28 if tiene_metrica_2 else 0.2,
        bargroupgap=0.18 if tiene_metrica_2 else 0.0,
        margin=dict(l=52, r=28, t=max(76, 52 + fs_etiqueta_barra), b=56),
        xaxis=dict(
            type='category',
            tickfont=dict(size=base_font_size, color="#e2e8f0"),
...
def render_perfilado_manual_panel(
    ...
    if pareto_activo_prev:
        with col_chart:
            st.markdown(
                '<div style="width:100%;max-width:100%;overflow:hidden;margin-left:-8px;">',
                unsafe_allow_html=True,
            )
