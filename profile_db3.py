...
def fig_ranking_barras(
    ...
    # Plotly exige height/width >= 10; con viewport muy bajo y modo código los márgenes + altura fallaban.
    vph = max(RESERVA_VERTICAL_REF + CHART_HEADROOM_COD_PX + 240, vph)
...
if df is not None:
    with st.sidebar:
        ...
        if st.session_state.get("lri_sidebar_perfil_estandar") == PERFIL_ESTANDAR_ROTACION_5:
            if st.button(
        ...
        perfil_estandar = st.selectbox(
            "Plantilla estándar",
            options=[PERFIL_ESTANDAR_DEFAULT, PERFIL_ESTANDAR_ROTACION_5],
            index=0,
            key="lri_sidebar_perfil_estandar",
            help="Rotación: cinco paneles (Cod., Cat., Sub., Proveedor, País).",
        )
        if perfil_estandar == PERFIL_ESTANDAR_ROTACION_5:
